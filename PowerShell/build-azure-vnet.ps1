...
## Build new RG
$rg_name = "$gdot_environment-$app_name-$($lookup.$gdot_environment.$location.reg_short)-$seq_number-VNET-RG"
$rg_test = ''
...
foreach($snet in $subnets) {
    if ($snet[2] -eq "false") {
        if ($rt_object_check -eq 0) {
            $rt_name = "$generic_name-$seq_number-RT"
        ...
        if ($rt_object_apim_check -eq 0) {
            $rt_name_apim = "$generic_name-$seq_number-APIM-RT"
        ...
        if ($rt_object_agw_check -eq 0) {
            $rt_name_agw = "$generic_name-$seq_number-AGW-RT"
...
$hub_rt_obj | Add-AzRouteConfig -Name "$generic_name-$seq_number-RT" -AddressPrefix $vnet_prefix -NextHopType "VirtualAppliance" -NextHopIpAddress $lookup.$gdot_environment.$location.nexthop
